...
const SPEC_KEYS = ["measurementCapability", "accuracyAndPerformance", "dataAndInterface", "applications"];

function SpecCard({ sectionKey, delay }: { sectionKey: string; delay: number }) {
	const { t } = useTranslation(["total-station-esurvey"]);
	const group = t(`total-station-esurvey:${sectionKey}`, {
		returnObjects: true,
	...
	return (
		<FadeUp delay={delay}>
			<article className="h-full flex flex-col gap-5 rounded-[20px] bg-surface hairline card-shadow p-7 sm:p-8 transition-all duration-500 hover:card-shadow-lift">
				...
				<h3 className="text-xl sm:text-2xl font-light tracking-tight text-ink">
					{group.headline}
				</h3>
